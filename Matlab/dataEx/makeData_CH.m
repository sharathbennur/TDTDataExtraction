function [data_b] = makeData_CH(file_in)
        ...
        elseif strcmp(data_id.ecodes(j,2),'comp')
            ...
            if strcmp(data_id.ecodes(j,1),'lev_rel') % Compute release time
                for tt=1:tn
                    ...
                    sof = data_b.ecodes.data(tt,gD_C('t1')); % sound off
...
%% Special section to interpret data.ecodes.curr_params for VocInChorus
% trials

% First save Voc-to-Chorus ratio, Chorus# used and offset between Chorus
% and Voc onsets in ms
c = size(data_b.ecodes.name,2);
data_b.ecodes.name(c+1) = {'VCR'};
data_b.ecodes.data(:,c+1) = nan;
data_b.ecodes.name(c+2) = {'Chorus#'};
data_b.ecodes.data(:,c+2) = nan;
data_b.ecodes.name(c+3) = {'VocTime'};
data_b.ecodes.data(:,c+3) = nan;
for tt=1:tn
    tp = data_b.params{tt};
    di = strfind(tp,'&');
    if strcmp(tp(1:2),'HT')
        data_b.ecodes.data(tt,c+1) = str2double(tp(di(2)+1:end));
        data_b.ecodes.data(tt,c+3) = str2double(tp(3:di(1)-1));
    end
    data_b.ecodes.data(tt,c+2) = str2double(tp(di(1)+1));
end
