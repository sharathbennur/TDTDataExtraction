function combine_TN(data_b,sortNum,varargin)
...
data_n_folder = 'C:\Lab\Data\Mat\Nrl\ALB\';
data_b_folder = 'C:\Lab\Data\Mat\Beh\ALB\';
data_f_folder = 'C:\Lab\Data\Mat\Final\ALB\';

if nargin>1
    save(strcat(data_n_folder,'N_',CurrentBlock,'_',...
        num2str(CN),'.',num2str(sortNum),'.mat'),'data');
    ...
    num2str(CN),'.',num2str(sortNum),'.mat');
    
    save(file_out,'data');
    disp(strcat('4. Saved final data file: ',file_out));
else
    save(strcat(data_n_folder,'N_',CurrentBlock,'_',...
        num2str(find(CN)),'.',CurrentEvent(end),'.mat'),'data');
    disp('2. Saved neural data file');
    save(strcat(data_b_folder,'B_',CurrentBlock),'data_b')
    disp('3. Converted and saved behavior data file');
    ...
    % Filename
    file_out = strcat(data_f_folder,'C_',CurrentBlock,'_',...
    num2str(find(CN)),'.',CurrentEvent(end),'.mat');
