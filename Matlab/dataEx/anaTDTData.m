...
elseif af==4 % Voc/Chorus
    La = getTimeStamps(TDTData.Aud,TDTData.Audfs,7);
    NData.codes.name(1,Lc+1) = {'chorus_on'}; % init names
    NData.codes.data(1:nt,Lc+1) = nan; % init data
end
La = La./TDTData.Audfs;
if size(La,1)>0 % make sure there are valid trials
    ...
    elseif af==3 % Tone/Noise
        ...
        % NOTE: can specify the chorus code seperately or use TN code for finding aud
        % on/offsets and just use the first Lat timestamp - which is chorus
        % onset
    elseif af==4 % Chorus
        for i=1:nt % no. trials
            % look between
...
% plot(xa,TDTData.Aud*100,'r')
% plot(La(:,1),10,'r*','MarkerSize',10)
% %
