...
if ~isfield(data.epocs, epoc)
    f = fieldnames(data.epocs);
    error([epoc ' is not a valid epoc event, valid events are: ' strjoin(f', ', ')])
end
...
if ~isempty(VALUES)
    ...
    % preallocate
    time_ranges = zeros(2, length(d.data));
    
    % find valid time ranges
    time_range_ind = 0;
    for j = 1:length(d.data)
        if any(d.data(j) == VALUES)
            time_range_ind = time_range_ind + 1;
            time_ranges(:, time_range_ind) = [d.onset(j); d.offset(j)];
        end
    end
...
if ~isempty(TIME)
    ...
    % create filter string
    filter_string = sprintf('TIME: %s [%.1f:%.1f];', epoc, t1, t2);
end
...
if ~isempty(data.snips)
    ...
    for i = 1:length(n)
        ...
        data.snips.(n{i}).chan = data.snips.(n{i}).chan(keep);
        data.snips.(n{i}).sortcode = data.snips.(n{i}).sortcode(keep);
    end
end
